...
//       const response = await axios.get(`http://18.117.249.93:8080/prizes?date=${formattedDate}`);
...
//               <span>MRP: </span>
//               <span className="text-dark text-monospace font-weight-bolder pr-2" style={{ fontSize: '1.5em' }}> 70/- </span>
//               <span>Draw date: </span>
...
const TodayResults = () => {
  ...
  const fetchPrizes = async () => {
    try {
      ...
      // Ensure response.data is an array
      if (response.data.prizes.length > 0) {
        const sortedPrizes = response.data.prizes.sort((a, b) => a.position.localeCompare(b.position));
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      checkTimeAndFetchData();  // Check every minute after 7:00 PM
    }, 60000);
